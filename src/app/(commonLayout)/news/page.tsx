                ...
                <div className="flex items-center justify-between mb-4">
                    ...
                    {error === 'offline' && (
                        <span className="text-xs text-amber-600">
                            {L('Previewing mock data — add app/api/news/route.js to use your API.', '目前顯示範例數據 — 新增 app/api/news/route.js 以使用你的 API。')}
                        </span>
                    )}
                </div>
                ...
                <div className="grid md:grid-cols-3 gap-4">
                    {(items || []).map((n) => (
                        <article key={n.id} className="rounded-xl border overflow-hidden flex flex-col">
                            {n.image ? (
                                <img src={n.image} alt={L(n.titleEN, n.titleZH).toString()} className="w-full h-36 object-cover" />
                            ) : (
                                <div className="w-full h-36 bg-gradient-to-br from-slate-200 to-slate-100 flex items-center justify-center">
                                    <span className="text-slate-500 text-sm">{L('Text Update', '文字更新')}</span>
                                </div>
                            )}
                            <div className="p-4 flex-1 flex flex-col">
                                ...
                                {(n.bodyEN || n.bodyZH) && (
                                    <p className="text-sm text-slate-700 line-clamp-3">{L(n.bodyEN || '', n.bodyZH || '')}</p>
                                )}
                            </div>
                        </article>
                    ))}
