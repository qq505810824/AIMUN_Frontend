...
const baseOptions = {
    ...
    headers: new Headers({
        ...
        // 'Accept': ContentType.json // Removed this line
    }),
    redirect: 'follow'
};
...
const baseFetch = <T>(
    ...
    // Dynamically set Accept header to application/json if:
    // 1. The request's Content-Type is application/json
    // 2. No Accept header is already specified in fetchOptions
    // This ensures that we request JSON responses for JSON requests,
    // without interfering with requests that expect other content types (e.g., file downloads).
    if (
        options.headers.get('Content-Type') === ContentType.json &&
        !options.headers.has('Accept')
    ) {
        options.headers.set('Accept', ContentType.json);
    }
...
export const ssePost = (
    ...
    options.headers.set(
        'Authorization',
        `Bearer ${process.env.NEXT_PUBLIC_SPEAKING_CONVERSATION || 'app-9d0QgfzhD6Xc0GEGZySmF8wx'}`
    );
